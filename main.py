...
# создаем класс на основе класса BaseModel из библиотеки pydantic
...
# метод срабатывает при обращении по адресу http://127.0.0.1:8000 (локально)
# и http://158.160.48.28:8000 (если извне)
@app.get("/")
async def root():
    #  вписываем docstring для документации по нашему API
    """ Метод возвращает JSON-объект, содержащий сообщение
    'Курс Программная инженерия, 2023г. Группа 2.11'  """
...
async def cont():
    #  вписываем docstring для документации по нашему API
    """ Метод возвращает JSON-объект, содержащий контекстные данные,
    которые используем модель """
...
async def predict(item: Item):
    #  вписываем docstring для документации по нашему API
    """ Метод возвращает результаты работы модели
    timpal0l/mdeberta-v3-base-squad2 на заданный
    пользователем вопрос, основываясь на контексте, который скормлен в модель.
    Метод возвращает JSON объект содержащий ответ модели и score ответа."""
    # возвращаем результат работы модели по вопросу, который пользователь
    # ввел в POST запросе (в переменной item.text
